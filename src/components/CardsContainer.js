...
const CardsContainer = ({ items }) => {
    return (
        <div className='grid gap-10  grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 my-5 text-center'>
            {
                items.map((item, idx) => <Link to={item?.link} key={idx} className={`text-2xl p-5 rounded-lg shadow-md hover:tracking-widest hover:shadow-2xl duration-500 ease-in-out flex cursor-pointer ${item.bgColor ? item.bgColor : 'bg-accent'}`}>
